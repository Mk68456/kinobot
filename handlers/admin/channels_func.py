...
from database.channels import add_channel_, delete_channel_t
from keyboards.admin.keyboard import (
    admin_markup,
    admin_return_markup,
    allow_channel_markup,
    allow_channel_delete_markup,
)
...
async def delete_channel_func(message: types.Message, state: FSMContext):
    if message.text == "Назад":
        ...
        await bot.send_message(
            message.chat.id, "Удаление канала было отменено", reply_markup=types.ReplyKeyboardRemove()
        )
        await bot.send_message(
            message.chat.id,
        ...
        await bot.send_message(
            message.chat.id, f"Удалить канал {message.text} ?", reply_markup=allow_channel_delete_markup()
        )
...
async def add_new_channel_post_handler(message: types.Message, state: FSMContext):
    try:
        ...
        await bot.send_message(
            message.chat.id,
            f"Добавить канал {message.forward_from_chat.title}  ?",
            reply_markup=allow_channel_markup(),
        )
    except:
        await bot.send_message(
            message.chat.id, "Отправьте пост из канала :", reply_markup=admin_return_markup()
        )
        await Admin_.add_channel.set()


@dp.callback_query_handler(lambda call: call.data in ("add_yes", "add_no"), state=Admin_.add_channel)
@dp.callback_query_handler(lambda call: call.data in ("del_yes", "del_no"), state=Admin_.delete_channel)
async def return_channel_handler(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.data.startswith("add_"):
        if call.data[4:] == "yes":
            ...
            await state.finish()
            await bot.send_message(
                call.message.chat.id, "Канал успешно добавлен !", reply_markup=admin_markup()
            )
        else:
            await state.finish()
            await call.answer("Вы вернулись в админ-панель")
    ...
    if call.data.startswith("del_"):
        if call.data[4:] == "yes":
            async with state.proxy() as data:
                ...
                await bot.send_message(
                    call.message.chat.id, "Канал успешно удален !", reply_markup=types.ReplyKeyboardRemove()
                )
                await bot.send_message(call.message.chat.id, "Админ-панель", reply_markup=admin_markup())
            await call.message.delete()
            await state.finish()
