...
async def _render_stats_menu(chat_id: int):
    ...
    marked_films, marked_episodes = progress.totals()
    text = (
        ...
        f"Отмечено просмотренных фильмов: {marked_films}\n"
        f"Отмечено просмотренных серий: {marked_episodes}"
    )
...
async def _render_users_page(chat_id: int, page: int):
    ...
    await bot.send_message(
        chat_id,
        f"<strong>Пользователи (стр. {page + 1}) :</strong>\n\n" + "\n".join(lines),
...
async def stat_user_lookup_input_handler(message: types.Message, state: FSMContext):
    ...
    lines.append(f"Личные отметки: {episodes} серий, {films} фильмов.\n")
    lines.append("<strong>Открытые карточки фильмов/сериалов :</strong>")
    if watches:
        for title, numb, created_at in watches:
            lines.append(f"• {escape(str(title)[:120])} (код {numb}) — {created_at.split('T')[0]}")
    else:
        lines.append("нет данных")
    lines.append("\n<strong>Поисковые запросы :</strong>")
    if searches:
        for query, created_at in searches:
            lines.append(f"• «{escape(str(query)[:120])}» — {created_at.split('T')[0]}")
    else:
        lines.append("нет данных")
    ...
    await bot.send_message(
        ...
        reply_markup=buttons(
            [
                ("✅ Какие серии и фильмы просмотрены", f"watchstats:user:{user_id}:0"),
                ("К статистике", "stat_menu_back"),
            ]
        ),
    )
