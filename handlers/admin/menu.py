from database.channels import get_all_channels_title
from database.movie_queries import get_all_movies
...
async def admin_call_handler(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.data == "bot_stat":
        ...
        from .stats_handlers import _render_stats_menu

        await _render_stats_menu(chat_id)

    if call.data == "add_cod":
        await bot.delete_message(chat_id, message_id=call.message.message_id)
        await bot.send_message(
            chat_id, "<strong>Что вы хотите добавить ?</strong>", reply_markup=content_type_choice_markup()
        )
    ...
    if call.data == "send_func":
        ...
        await bot.send_message(
            chat_id,
            "<strong>Отправьте или Перешлите текст для рассылки :</strong>",
    ...
    if call.data == "add_channel":
        await bot.delete_message(chat_id, message_id=call.message.message_id)
        await bot.send_message(
            chat_id, "<strong>Отправьте ссылку на канал:</strong>", reply_markup=admin_return_markup()
        )
    ...
    if call.data == "delete_channel":
        ...
        await bot.send_message(
            chat_id,
            "<strong>Выберите канал который хотите удалить :</strong>",
            reply_markup=get_delete_channels(get_all_channels_title()),
        )
    ...
    if call.data == "delete_movie":
        ...
        await bot.send_message(
            chat_id,
            "<strong>Выберите фильм/сериал который хотите удалить :</strong>",
            reply_markup=get_delete_movies(get_all_movies()),
        )
    ...
    if call.data == "edit_movie":
        ...
        await bot.send_message(
            chat_id,
            "<strong>Выберите фильм/сериал который хотите изменить :</strong>",
            reply_markup=get_delete_movies(get_all_movies()),
        )
...
@dp.callback_query_handler(text="admin_return", state="*")
async def admin_return(call: types.CallbackQuery, state: FSMContext):
    await state.finish()
    await call.answer()
