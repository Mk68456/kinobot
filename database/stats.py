...
def log_search(user_id: int, query: str, found_count: int):
    _safe_execute(
        "INSERT INTO SearchEvents (user_id, query, found_count, created_at) VALUES (?,?,?,?)",
...
def log_watch(user_id: int, movie_number, movie_title: str):
    _safe_execute(
        "INSERT INTO WatchEvents (user_id, movie_number, movie_title, created_at) VALUES (?,?,?,?)",
...
def get_top_watched_movies(days: int = 7, limit: int = 10):
    cursor.execute(
        """SELECT movie_title, movie_number, COUNT(*) as cnt FROM WatchEvents
...
def get_top_searches(days: int = 7, limit: int = 15):
    cursor.execute(
        """SELECT query, COUNT(*) as cnt FROM SearchEvents
...
def get_users_list(limit: int = 30, offset: int = 0):
    ...
    if "username" in columns and "joined_at" in columns:
        cursor.execute(
            "SELECT id, username, joined_at FROM Users ORDER BY rowid DESC LIMIT ? OFFSET ?", (limit, offset)
        )
    else:
        cursor.execute(
            "SELECT id, NULL, NULL FROM Users ORDER BY rowid DESC LIMIT ? OFFSET ?", (limit, offset)
        )
    return cursor.fetchall()
...
def get_user_watch_history(user_id: int, limit: int = 20):
    cursor.execute(
        """SELECT movie_title, movie_number, created_at FROM WatchEvents
...
def get_user_search_history(user_id: int, limit: int = 20):
    cursor.execute(
        """SELECT query, created_at FROM SearchEvents
                       WHERE user_id=? ORDER BY created_at DESC LIMIT ?""",
        (user_id, limit),
    )
    return cursor.fetchall()
